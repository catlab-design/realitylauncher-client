...
pub async fn install_fabric(
...
) -> napi::Result<FabricInstallResult> {
    ...
    println!("[Fabric] Fetching loader versions from: {}", loader_url);

    let res = client
        .get(&loader_url)
        .send()
        .await
        .map_err(|e| napi::Error::from_reason(format!("Failed to fetch Fabric versions: {}", e)))?;

    let loaders: Vec<FabricLoaderVersion> = res
        .json()
        .await
        .map_err(|e| napi::Error::from_reason(format!("Failed to parse Fabric versions: {}", e)))?;

    if loaders.is_empty() {
        return Err(napi::Error::from_reason(format!(
            "Fabric does not support Minecraft {}",
            mc_version
        )));
    }
    ...
    println!("[Fabric] Selected loader version: {}", loader_version);
    ...
    println!("[Fabric] Fetching profile from: {}", profile_url);

    let profile_res = client
        .get(&profile_url)
        .send()
        .await
        .map_err(|e| napi::Error::from_reason(format!("Failed to fetch Fabric profile: {}", e)))?;

    let profile: FabricProfile = profile_res
        .json()
        .await
        .map_err(|e| napi::Error::from_reason(format!("Failed to parse Fabric profile: {}", e)))?;
    ...
    println!(
        "[Fabric] Version JSON saved to: {}",
        version_json_path.display()
    );

    // 5. Download libraries
    let libraries_dir = Path::new(&minecraft_dir).join("libraries");
    download_fabric_libraries(&profile.libraries, &libraries_dir).await?;
...
/// Download Fabric libraries to the libraries directory
async fn download_fabric_libraries(
    ...
    for lib in libraries {
        ...
        if parts.len() < 3 {
            println!("[Fabric] Skipping invalid library: {}", lib.name);
            continue;
        }
        ...
        if target_path.exists() {
            println!("[Fabric] Library exists: {}", file_name);
            continue;
        }
        ...
        println!("[Fabric] Downloading: {}", file_name);
        ...
        match client.get(&url).send().await {
            Ok(res) if res.status().is_success() => {
                ...
                println!("[Fabric] Downloaded: {}", file_name);
            }
            Ok(res) => {
                println!(
                    "[Fabric] Warning: Failed to download {} (status: {})",
                    file_name,
                    res.status()
                );
            }
            Err(e) => {
                println!("[Fabric] Warning: Failed to download {}: {}", file_name, e);
            }
        }
    }
...
#[napi]
pub async fn get_fabric_versions(mc_version: String) -> napi::Result<Vec<String>> {
    let client = get_client();
    let url = format!(
        "https://meta.fabricmc.net/v2/versions/loader/{}",
        mc_version
