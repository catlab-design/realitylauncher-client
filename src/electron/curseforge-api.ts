...
export async function searchCurseForge(
  ...
  if (filters.modLoaderType)
    params.set("modLoaderType", filters.modLoaderType.toString());
  if (filters.sortBy) params.set("sortBy", filters.sortBy);
  if (filters.pageSize) params.set("pageSize", filters.pageSize.toString());
  if (filters.index !== undefined)
    params.set("index", filters.index.toString());
...
): Promise<{ data: CurseForgeProject }> {
  const url = `${ML_API_URL}/curseforge/project/${projectId}`;
  return fetchJSON<{ data: CurseForgeProject }>(url);
}

/**
 * Get project description (HTML)
 */
export async function getCurseForgeDescription(
  projectId: number | string,
): Promise<{ data: string }> {
  const url = `${ML_API_URL}/curseforge/project/${projectId}/description`;
  try {
    return await fetchJSON<{ data: string }>(url);
  } catch (error: any) {
    // Fallback to direct proxy if backend fails or doesn't have the route yet
    if (error.message.includes("404")) {
      console.warn(
        `[CurseForge] Backend 404 for description, trying fallback proxy for project ${projectId}...`,
      );
      const fallbackUrl = `https://api.curse.tools/v1/cf/mods/${projectId}/description`;
      return fetchJSON<{ data: string }>(fallbackUrl);
    }
    throw error;
  }
}
 ...
 */
export async function getCurseForgeCategories(): Promise<{
 ...
 */
export async function getCurseForgeVersions(): Promise<{
  data: { version: string; versionString: string }[];
}> {
