...
export function registerCurseForgeHandlers(): void {
    ...
    ipcMain.handle("curseforge-search", async (_event, filters: any) => {
        try {
            ...
            console.log("[CurseForge] Search result cached:", result?.data?.length || 0, "items");
            return result;
        } catch (error: any) {
            ...
            if (result?.data) {
                ...
                for (const f of result.data) {
                    f.gameVersions?.forEach((v: string) => {
                        if (['fabric', 'forge', 'neoforge', 'quilt'].includes(v.toLowerCase())) {
                            loaders.add(v);
                        } else {
                            versions.add(v);
                        }
                ...
                console.log("[CurseForge] Available MC versions:", [...versions].slice(0, 20));
    ...
    ipcMain.handle("curseforge-prefetch", async () => {
        try {
            ...
            setCache(searchCache, getCacheKey("cf-search", { projectType: "modpack", pageSize: 20, sortBy: "downloads" }), modpacks);
            ...
            setCache(searchCache, getCacheKey("cf-search", { projectType: "mod", pageSize: 20, sortBy: "downloads" }), mods);

            console.log("[CurseForge] Prefetch complete:", modpacks?.data?.length || 0, "modpacks,", mods?.data?.length || 0, "mods");
            return { ok: true, modpacks: modpacks?.data?.length || 0, mods: mods?.data?.length || 0 };
        } catch (error: any) {
